...
int CameraConnect::OpenCamera()
{
    ...
    try {
        ...
        // Убираем биннинг камеры для более высокого разрешения
//        CIntegerParameter(cameraNodeMap, "BinningVertical").SetValue(2);
//        CIntegerParameter(cameraNodeMap, "BinningHorizontal").SetValue(2);

        qDebug() << cameraNodeMap.GetNode("LineSelector")->GetName();

        CEnumParameter(cameraNodeMap, "LineSelector").SetValue("Line3");
        CEnumParameter(cameraNodeMap, "LineSource").SetValue("ExposureActive");
        CEnumParameter(cameraNodeMap, "TriggerSource").SetValue("Software");
    ...
    try  {
        ...
        if(m_currentMode == "Software")
        {
            if (m_basler.WaitForFrameTriggerReady(1000, TimeoutHandling_Return)) {
                ...
                m_basler.RetrieveResult(1000, ptrGrabResult,TimeoutHandling_Return);
            }
        }
